...
type, public :: fBrickLam_element
  ...
  integer,               allocatable :: edge_connec(:)
...
pure subroutine extract_fBrickLam_element (elem, curr_status, node_connec, &
& edge_connec, layup, plyblks, interfs)
use fBrick_element_module,  only : fBrick_element
use fDelam8_element_module, only : fDelam8_element
  ...
  if(present(node_connec)) then
      if(allocated(elem%node_connec)) then
          allocate(node_connec(size(elem%node_connec)))
          node_connec = elem%node_connec
      end if
  end if
  
  if(present(edge_connec)) then
      if(allocated(elem%edge_connec)) then
          allocate(edge_connec(size(elem%edge_connec)))
...
pure subroutine set_fBrickLam_element (elem, NPLYBLKS, node_connec, edge_connec, &
& layup, istat, emsg)
...
use fDelam8_element_module, only : set
  ...
  integer,  intent(in)  :: edge_connec(NPLYBLKS*NEDGE)
  ...
  ! set edge connec
  allocate(el%edge_connec(NPLYBLKS*NEDGE))
  el%edge_connec = edge_connec
